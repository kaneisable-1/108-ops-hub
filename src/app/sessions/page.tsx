...
function SessionsContent() {
  ...
  return (
    <div className="min-h-screen">
      {/* Header */}
      <div className="sticky top-0 z-10 px-4 py-3 pt-safe" style={{ background: 'var(--surface-primary)', borderBottom: '1px solid var(--surface-border)' }}>
...
function SessionDetailExpanded({
  ...
  const sentimentBadge: Record<string, { bg: string; label: string }> = {
    green: { bg: 'bg-emerald-500/15 text-emerald-400', label: 'Green — No issues' },
    yellow: { bg: 'bg-amber-500/15 text-amber-400', label: 'Yellow — Needs discussion' },
    red: { bg: 'bg-red-500/15 text-red-400', label: 'Red — No-go' },
  }

  const sentiment = session.coach_sentiment ? sentimentBadge[session.coach_sentiment] : null

  return (
    <div className="card p-4 mt-1 space-y-4 border-l-4 border-brand-500/30">
      {/* Sentiment detail */}
      {sentiment && (
        <div>
      ...
      {session.raw_notes && (
        <div>
          <h4 className="text-xs font-semibold uppercase mb-1" style={{ color: 'var(--text-muted)' }}>Raw Notes</h4>
      ...
      {session.parsed_notes && session.ai_parsed_at && (
        <div>
          <h4 className="text-xs font-semibold uppercase mb-1" style={{ color: 'var(--text-muted)' }}>
            AI-Parsed Notes
            <span className="ml-1 normal-case" style={{ color: 'var(--text-muted)' }}>
      ...
      {session.note_mode === 'extended' && (
        <>
          {session.drills_performed && session.drills_performed.length > 0 && (
            <div>
              <h4 className="text-xs font-semibold uppercase mb-1" style={{ color: 'var(--text-muted)' }}>Drills</h4>
              <div className="flex flex-wrap gap-1">
                {session.drills_performed.map((d, i) => (
                  <span key={i} className="badge bg-brand-500/15 text-brand-400 text-xs">{d}</span>
                ))}
              </div>
            </div>
          )}
          {session.athlete_effort_rating && (
            <div>
              <h4 className="text-xs font-semibold uppercase mb-1" style={{ color: 'var(--text-muted)' }}>Effort Rating</h4>
              <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>{session.athlete_effort_rating}/5</p>
            </div>
          )}
          {session.injury_notes && (
            <div>
              <h4 className="text-xs font-semibold uppercase text-red-400 mb-1">Injury Notes</h4>
              <p className="text-sm text-red-400">{session.injury_notes}</p>
            </div>
          )}
        </>
      )}
      ...
      {session.is_exit_eval && session.exit_eval && (() => {
        ...
        return (
          <div className="pt-3" style={{ borderTop: '1px solid var(--surface-border)' }}>
            <h4 className="text-xs font-semibold uppercase text-purple-400 mb-2">Exit Evaluation</h4>
            <div className="grid grid-cols-2 gap-2 text-xs" style={{ color: 'var(--text-secondary)' }}>
              {evalData.progress_rating != null && (
                <div><span style={{ color: 'var(--text-muted)' }}>Progress:</span> {String(evalData.progress_rating)}/5</div>
              )}
              {evalData.recommendation != null && (
                <div><span style={{ color: 'var(--text-muted)' }}>Rec:</span> {String(evalData.recommendation)}</div>
              )}
              {evalData.would_work_again != null && (
                <div><span style={{ color: 'var(--text-muted)' }}>Again:</span> {String(evalData.would_work_again)}</div>
              )}
            </div>
