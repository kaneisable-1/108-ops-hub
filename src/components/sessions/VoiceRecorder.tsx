...
export default function VoiceRecorder({ onTranscriptChange, transcript }: VoiceRecorderProps) {
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        ...
        {transcript && (
          <button
            type="button"
            onClick={handleClear}
            className="rounded-lg p-2 text-steel-400 hover:text-navy-500 hover:bg-steel-100 transition-colors"
          >
      ...
      {transcript && (
        <div className="rounded-lg bg-steel-50 p-3 text-sm text-steel-700 max-h-32 overflow-y-auto">
          {transcript}
        </div>
      )}
    </div>
